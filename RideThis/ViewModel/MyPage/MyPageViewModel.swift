...
class MyPageViewModel {
    ...
    func getTypeAverageby(records: [RecordModel], dataCase: RecordDataCase, periodCase: RecordPeriodCase) {
        switch dataCase {
        case .cadence:
            cadenceAvg = records.count > 0 ? (records.map{ $0.record_cadence }.reduce(0, +) / Double(records.count)).getTwoDecimal : 0
        case .distance:
            distanceAvg = records.count > 0 ? (records.map{ $0.record_distance }.reduce(0, +) / Double(records.count)).getTwoDecimal : 0
        case .speed:
            speedAvg = records.count > 0 ? (records.map{ $0.record_speed }.reduce(0, +) / Double(records.count)).getTwoDecimal : 0
        case .calories:
            caloriesAvg = records.count > 0 ? (records.map{ $0.record_calories }.reduce(0, +) / Double(records.count)).getTwoDecimal : 0
        }
    }
}
